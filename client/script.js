...
sayHelloButton.addEventListener('mouseover', makeButtonBlack);

const makeButtonWhite = (event) => {
    event.target.style["background-color"] = "#EFEFEF";
    event.target.style["color"] = "black";
}

sayHelloButton.addEventListener('mouseout', makeButtonWhite);
...
const ohMy = () => {
    ...
    axios.get(`http://localhost:3000/animals`).then( res => {
        ...
        res.data.forEach(object => {
            ...
            newPelement.textContent = object;

            documentBody.appendChild(newPelement);
        })
    });
}
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    let param = 'nate'
    axios.get(`http://localhost:3000/repeat/${param}`).then( res => document.querySelector('#repeat-text').textContent = res.data );
...
const getRequest = () => {
    ...
    axios.get(`http://localhost:3000/query-test?name="Nate&age=23"`).then(res => console.log(res.data));
    
}

const queryButton = document.querySelector('#query-button');
queryButton.addEventListener('click', getRequest);
...
/*
    ...
    Response returns an error 404, or if you add the ? at the end and do not send any parameters it just returns blanks.

    2: Send more than 1 query on the URL -- what happened? 
    
    It returns a message that states you sent more than 1 query alongside the queries.
    {message: "You sent more than 1 query!", queries: {name: "\"Nate", age: "23\""}}
*/
